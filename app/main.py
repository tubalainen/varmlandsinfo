...
USER_AGENT = "varmlandsinfo/1.0 (+https://github.com/tubalainen/varmlandsinfo)"
...
async def refresh() -> None:
    try:
        ...
        log.info("Hämtade %d evenemang (%d aktuella)", len(raw), len(events))
...
app = FastAPI(title="Värmlandsinfo", lifespan=lifespan)


@app.get("/api/events")
async def events():
    first_day = today().isoformat()
    # Filtrera bort tillfällen som passerat sedan senaste hämtningen
    result = []
    ...
    return {"updated": state["updated"], "error": state["error"], "today": first_day, "events": result}


@app.get("/api/health")
async def health():
    return {"ok": True, "events": len(state["events"]), "updated": state["updated"], "error": state["error"]}
